UpdateGetATTAINSData <- function(){
  ...
  GreenBay_FoxRiver <- EPATADA::TADA_FlagCoordinates(GreenBay_FoxRiver, clean_outsideUSA = "change sign", 
                       clean_imprecise = FALSE)
  
  GreenBay_FoxRiver <- GreenBay_FoxRiver[!is.na(GreenBay_FoxRiver$LongitudeMeasure),]
  ...
  GreenBay_FoxRiver <- GreenBay_FoxRiver %>%
    ...
    WQP_with_ATTAINSonly <- EPATADA::TADA_GetATTAINS(GreenBay_FoxRiver, fill_catchments = FALSE, 
                                                     return_sf = TRUE)
